...
import { getPuArrivalHead2 } from '@/api/puprint'
export default {
  name: `puprint`,
  data() {
    return {
  ...
  computed: {
    startDateStr() {
      if (this.form.dBeginDate == '' || this.form.dBeginDate == null) return dayjs(new Date()).format('YYYY-MM-DD')
      return this.formatDate(this.form.dBeginDate)
    },
    endDateStr() {
      if (this.form.dEndDate == '' || this.form.dEndDate == null) return dayjs(new Date()).format('YYYY-MM-DD')
